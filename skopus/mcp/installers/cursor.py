...
Writes (or merges into) ~/.cursor/mcp.json's ``mcpServers`` block so
Cursor discovers the Skopus stdio MCP server on next launch.
...
def install_cursor_mcp(home: Path | None = None) -> dict:
    """Add (or refresh) the Skopus entry in Cursor's mcpServers block.
...
def uninstall_cursor_mcp(home: Path | None = None) -> dict:
    """Remove the Skopus entry from Cursor's mcpServers block.
